...
To pick up a metaphor that's been used a couple of times [in the pamphlets](https://litlab.stanford.edu/LiteraryLabPamphlet11.pdf) - if we think of ourselves as literary "astronomers" (the corpora are the night sky; the MPI programs on the cluster are bespoke little radio telescopes that harvest out some kind of observational data about the literary universe) - then, how fine-grained or general-purpose should the observations be? Should we more or less begin with specific, fully-formed hypotheses - often growing out of analog reading experiences, I think - and program the telescope to point at one particular little piece of the sky and observe at a specific frequency where we think there might be something of interest? Or should we begin with a kind of meta-hypothesis - tune the telescope to listen for an entire _class_ of objects - stars at a particular stage of development, exoplanets, galaxies of a particular type - and sweep out big arcs in the sky, scan the sky at a particular frequency and see what we find? What's our philosophy of feature extraction, and, in a more general sense, of intellectual _process_ - do we begin with specific literary-critical hypotheses and test them directly, quickly, surgically, or do we begin with a more diffuse critical orientation, an interest in a class of questions, gather up lots of data, and then let specific lines of inquiry emerge around the salient features that pop out in the process of analysis?

It's not that we can escape hypotheses just collect everything and fall back into a kind of hands-off, research-question-agnostic more of exploratory data analysis - there has to be some narrowing-down, some reductive process, or else we'd just re-collect the entire corpus as it sits on the scratch disk on Sherlock, like Borges perfect map that becomes as big as the world. but what's the most useful "breadth" of hypothesis? how big or small, diffuse or concrete?
...
But, back to the pragmatic question of where to put the data - in the last few months I've stumbled into an approach that seemed a little weird at first, but which, as I've gotten further along on a couple of projects, I like more and more - just put everything in [SQLite](https://sqlite.org/).

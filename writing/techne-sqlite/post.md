...
to pick up a metaphor that's been used a couple of times in the pamphlets - if we think of ourselves as literary "astronomers" (the corpora are the night sky; the MPI programs on the cluster are bespoke little telescopes) - then how should we go about "observing" the literary unviverse? do we read books, notice things, and then write programs that observe those things directly - point the telescope at one specific object in the sky, a particular phenomenon of literary history or form, and try to capture information that can notch directly into an existing critical narrative? or do we begin with an observational _method_ - a frequency or wavelength of interest, a new type of sensor, a hypothesis that a particular type of data will shed light on literary critical questions, and then sweep out big arcs in the sky and see what pops out, like comet hunters who look for stars that "move" against the backdrop of the surrounding starfield? when we sit down at the keyboard - to what extent should we know what we're looking for?


n interest in a _type_ of data instead of data about something in particular, and sweep out big arcs in the sky and see what pops out, like comet hunters who look for stars that "move" against the backdrop of the surrounding starfield? what's out philosophy of feature extraction, and more broadly, of intellectual _process_? do we begin with specific hypotheses about literature and test them directly, quickly, surgically? or do we start with a sort of meta-hypothesis, a hypothesis that a particular type of information will shed light on literary-critical questions, and scoop up lots of data with the hope that lines of inquiry emerge around salient features that pop out in the process of analysis? when we sit down at the keyboard - to what extent should we know what we're looking for?

this isn't clear to me. both seem to have risks. start too narrow, and you might miss a more interesting version of the question; start to broad, and you might get lost in a sea of leads and never wrestle things back into contact with literature, as opposed to just language or information in a general sense.

## SQLite

Anyway though, to come back to the practical question of where to put the data - in the last few months I've stumbled into an approach that seemed a little weird at first, but which, as I've gotten further along on a couple of projects, I like more and more - when in doubt, just put everything in [SQLite](https://sqlite.org/).

This seemed strange, I think, because of some common misconceptions that I had about SQLite from my previous life when I spent most of my time doing application development, not scientific / statistical programming. At first blush SQLite can seem sort of like a toy compared to regular databases Postgres or MySQL. There's no server - the entire database is stored in a single flat file, and transactions are implemented with unix file locks, which obviously has implications for concurrency that can be showstoppers for, say, a busy web application that needs to do lots of concurrent writes. In my head it was something you might use in mobile apps or embedded systems, for relatively small amounts of data in constrained environments where a full-service database would be overkill.
...
_Thanks to Ryan Heuser and Franco Moretti for reading drafts of this._
